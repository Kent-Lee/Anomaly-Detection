...
plot_minute <- function(title, grouped_train, grouped_tests)
{
  ...
  plot(grouped_train[[1]]$Time, grouped_train[[1]]$mean, type = "l", main = title, yaxt = "n", lwd = 2, panel.first = grid(), xlab = "", ylab = "Average Global Active Power", ylim = c(0.5,8))
...
plot_month <- function(title, grouped_data)
{
  ...
  plot(0, type = "n", xaxt = "n", yaxt = "n", panel.first = grid(), main = title, xlab = "", ylab = "Average Global Active Power", xlim = c(1,12), ylim = c(0.5,8))
...
{
  png(paste0("./figs/" ,paste0(title, ".png")), width = 1920, height = 1080)
  plot(0, type = "n", xaxt = "n", yaxt = "n", panel.first = grid(), main = title, xlab = "", ylab = "Average Global Active Power", xlim = c(1,4), ylim = c(0.5,8))
  colour <- c(rgb(0, 0, 0, 0.5), rgb(1, 0, 0, 0.5), rgb(0, 1, 0, 0.5), rgb(0, 0, 1, 0.5), rgb(0, 1, 1, 0.5), rgb(1, 0, 1, 0.5))
  for (i in 1:length(grouped_data)) {
...
{
  png(paste0("./figs/" ,paste0(title, ".png")), width = 1920, height = 1080)
  plot(test$Time, test$Global_active_power, panel.first = grid(), main = title, xlab = "", ylab = "Global Active Power", pch = 16, col = rgb(0, 0, 0, 0.5))
...
plot_collective_anomaly <- function(title, test, window_size, threshold)
{
  ...
  plot(1:nrow(test), test$Global_active_power, panel.first = grid(), main = title, xlab = "Observations", ylab = "Global Active Power", pch = 16, col = rgb(0, 0, 0, 0.5))
...
plot_logLike <- function(title, logLike)
{
  png(paste0("./figs/" ,paste0(title, ".png")), width = 1920, height = 1080)
  plot(logLike, type = "o", xaxt = "n", panel.first = grid(), main = title, xlab = "", ylab = "Log Likelihood", xlim = c(1,6), pch = 16)
...
# there must be a better way to do this, but I'm lazy and I don't want things to be too complicated
...
plot_logLike("Wednesday Morning Log Likelihood", wed.m.hmm)
plot_logLike("Wednesday Afternoon Log Likelihood", wed.a.hmm)
plot_logLike("Wednesday Night Log Likelihood", wed.n.hmm)
plot_logLike("Saturday Morning Log Likelihood", sat.m.hmm)
plot_logLike("Saturday Afternoon Log Likelihood", sat.a.hmm)
plot_logLike("Saturday Night Log Likelihood", sat.n.hmm)
